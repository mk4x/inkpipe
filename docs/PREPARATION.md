...
| 19 | Verbosity | Three named levels: Verbatim, Cleaned, Expanded. **Superseded by decision 32**: the output is the page tidied, not the page plus an appendix | A slider has no testable semantics. Three levels do. The levels survive; what "Expanded" produces changed. |
...
| 32 | What the output IS | A cleaner version of the page, not an explained one. Tidy the transcript, complete a standard set the page leaves open, flag wrong arithmetic beside itself, cut diagrams out. Never correct silently, and always keep the raw transcript | Measured against the owner on real pages: *"there is almost no editing or addition to the notes i write... not filling it with new information"*. An appendix of three sentence essays makes the note longer and fills it with material the student did not write, which helps a reader who knows nothing and is useless to the person who was in the room. Cleaning reverses the founding verbatim promise, so it is bounded by growth, word loss and repetition, and a rejected tidy-up leaves the raw transcript standing. See [adr/0006](adr/0006-clean-notes-not-explained-notes.md). |
